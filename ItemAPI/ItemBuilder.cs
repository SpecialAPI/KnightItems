...
namespace ItemAPI
{
    public static class ItemBuilder
    {
        ...
        private static void InitPassiveModifiers(PickupObject item)
        {
            if (item is Gun)
            {
                if ((item as Gun).passiveStatModifiers == null)
                {
                    (item as Gun).passiveStatModifiers = EMPTY_MODIFIER;
                }
            }
            else if (item is PassiveItem)
            {
                if ((item as PassiveItem).passiveStatModifiers == null)
                {
                    (item as PassiveItem).passiveStatModifiers = EMPTY_MODIFIER;
                }
            }
            else if (item is PlayerItem)
            {
                if ((item as PlayerItem).passiveStatModifiers == null)
                {
                    (item as PlayerItem).passiveStatModifiers = EMPTY_MODIFIER;
                }
            }
        }
        ...
        private static StatModifier[] EMPTY_MODIFIER = new StatModifier[0];
    }
}
